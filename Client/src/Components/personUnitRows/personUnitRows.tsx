    ...
    return (
        <div className={styles.personUnit}>
            <div className={styles.header}>
                ...
                <div className={styles.nameContainer} onClick={()=>{changeActive("name")}}><div className={styles.name}>{"Полное имя"}</div>
                    <Arrow currentSortObject={currentSortObject} columnName={"name"}/>
                </div>
                <div className={styles.domainContainer} onClick={()=>{changeActive("domain")}}><div className={styles.domain}>{"Учётная запись"}</div>
                    <Arrow currentSortObject={currentSortObject} columnName={"domain"}/>
                </div>
                <div className={styles.emailContainer} onClick={()=>{changeActive("email")}}><div className={styles.email}>{"Электронная почта"}</div>
                    <Arrow currentSortObject={currentSortObject} columnName={"email"}/>
                </div>
                <div className={styles.groupsContainer}><div className={styles.groups}>{"Группы"}</div>
                    <div className={styles.arrow}></div>
                </div>
                <div className={styles.phoneContainer} onClick={()=>{changeActive("phone")}}><div className={styles.phone}>{"Номер телефона"}</div>
                    <Arrow currentSortObject={currentSortObject} columnName={"phone"}/>
                </div>
            </div>
            {data && data.users.map((person:TClient, index: any)=>{
                return <PersonRow person={person} index={index} key={uuidv4()} isChecked={isChecked}/>
            })}
        </div>
    );
};
