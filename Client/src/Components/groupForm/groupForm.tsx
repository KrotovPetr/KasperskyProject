...
const GroupForm = () => {
    ...
    const [createNewGroup,{data, error, isLoading}] = useCreateGroupMutation();
    ...
    return (
        <form className={styles.groupForm} onSubmit={ handleSubmit(onSubmit)}>
            {error && <p className={styles.warningP}>This group has already exist!</p>}
            <input placeholder={"Group Name"} {...register("name")} className={styles.input}/>
